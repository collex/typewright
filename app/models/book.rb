...
#	end
	...
	def self.setup_doc(doc)
		img_thumb = doc.thumb()
		num_pages = doc.get_num_pages()

		title = XmlReader.read_metadata(doc.book_id())
		title_abbrev = title.length > 32 ? title.slice(0..30)+'...' : title

		return { :doc_id => doc.id, :num_pages => num_pages,
			:img_thumb => img_thumb, :title => title, :title_abbrev => title_abbrev
		}
	end

	def self.setup_page(doc, page)
		page = (page == nil) ? 1 : page.to_i

		img_thumb = doc.img_thumb(page)
		img_full = doc.img_full(page)
		img_size = doc.img_size(page)
		num_pages = doc.get_num_pages()

		src = XmlReader.read_gale(doc.book_id(), page)
		...
		pgs.times { |pg|
			src = XmlReader.read_gale(doc.book_id(), pg+1)
			src.each {|box|
				words[box[:word]] = words[box[:word]] == nil ? 1 : words[box[:word]] + 1
			}
		}
		...
		recs = Line.find_all_by_document_and_page(doc.id, page)
		changes = {}
		recs.each {|rec|
			key = "#{rec[:line]}"
			if changes[key]
		...
		return { :doc_id => doc.id, :page => page, :num_pages => num_pages, :img_full => img_full,
